...
function RadarChart(id, data, options) {
	...
	axis.append("line")
		.attr("x1", 0)
		.attr("y1", 0)
		.attr("x2", function(d, i){ return rScale(maxValue*1.03) * Math.cos(angleSlice*i - Math.PI/2); })
		.attr("y2", function(d, i){ return rScale(maxValue*1.03) * Math.sin(angleSlice*i - Math.PI/2); })
		.attr("class", "line")
		.style("stroke", "#CDCDCD")
	...
	blobWrapper
		...
		.on('mouseover', function (d,i){
			...
			d3.select(this)
				.transition().duration(200)
				.style("fill-opacity", cfg.opacityArea*2);	
		})
...
function RadarChartUpdate(id, data, options) {
	...
	//Create a wrapper for the blobs	
	var blobWrapper = svg.select(".radarWrapper")
		.data(data);
